 ...
 *
 * Requisitos por estudiante:
 *   - parent_email          → email de login del padre (único por familia)
 *   - parent_document_number → CC del acudiente = contraseña inicial (mín. 6 chars)
 *
 * Lógica por registro:
 *   1. Email ya existe como parent Y ya vinculado al estudiante → SKIPPED
 *   2. Email ya existe como parent pero NO vinculado            → LINKED (solo vincula)
 *   3. Email no existe                                          → CREATED (Firebase + DB + link)
 *
 * El padre puede tener múltiples hijos (hermanos con el mismo parent_email).
 */
router.post('/parents/bulk-create', ...auth, roles('school_admin', 'coordinator'), async (req, res, next) => {
  try {
    ...
    // Cache en memoria para evitar duplicar la creación cuando varios estudiantes
    // comparten el mismo email de padre (hermanos)
    const emailToParentId = {};

    // Precargar padres que ya existen en este colegio
    ...
    const limit = pLimit(5);   // máximo 5 llamadas simultáneas a Firebase
    ...
    const tasks = students.map(student => limit(async () => {
      ...
      try {
        ...
        if (parentDbId) {
          // El padre ya tiene cuenta — verificar si ya está vinculado a este estudiante
          ...
          // Existe el padre, falta el vínculo
        ...
        // El padre no existe — crear en Firebase
        let firebaseUid;
        try {
          ...
          if (fbErr.code === 'auth/email-already-exists') {
            // Email existe en Firebase pero no en nuestra DB — sincronizar
            try {
        ...
        emailToParentId[email] = parentDbId;   // actualizar cache

        // Vincular con el estudiante
        await db('student_parents').insert({ student_id: student.id, parent_id: parentDbId, relationship: 'acudiente' });
...
/**
 * Restablece la contraseña de un padre al CC registrado en el estudiante vinculado.
 * Si no se puede determinar el CC, usa DEFAULT_PASSWORD.
 */
router.post('/parents/:id/reset-password', ...auth, roles('school_admin', 'coordinator'), async (req, res, next) => {
  try {
    ...
    // Buscar CC del acudiente desde los estudiantes vinculados
